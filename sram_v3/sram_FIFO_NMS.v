...
//       
//       Creation Date:  Sun May 21 10:27:07 2023
//       Version: 	r5p0
//
...
`ifdef POWER_PINS
module sram_FIFO_NMS (VDDCE, VDDPE, VSSE, CENYA, WENYA, AYA, DYA, CENYB, WENYB, AYB,
...
`else
module sram_FIFO_NMS (CENYA, WENYA, AYA, DYA, CENYB, WENYB, AYB, DYB, QA, QB, CLKA,
...
`endif
  ...
  reg [103:0] readLatch1;
  reg [103:0] shifted_readLatch1;
  reg [1:0] read_mux_sel1;
  reg LAST_CLKB;
  reg [25:0] QA_int;
  reg [25:0] QA_int_delayed;
  reg [25:0] QB_int;
  reg [25:0] QB_int_delayed;
  reg [25:0] writeEnable;
  real previous_CLKA;
  real previous_CLKB;
  ...
  reg NOT_AA2, NOT_AA1, NOT_AA0, NOT_DA25, NOT_DA24, NOT_DA23, NOT_DA22, NOT_DA21;
  reg NOT_DA20, NOT_DA19, NOT_DA18, NOT_DA17, NOT_DA16, NOT_DA15, NOT_DA14, NOT_DA13;
  ...
  reg NOT_AB6, NOT_AB5, NOT_AB4, NOT_AB3, NOT_AB2, NOT_AB1, NOT_AB0, NOT_DB25, NOT_DB24;
  ...
  reg NOT_TAA1, NOT_TAA0, NOT_TDA25, NOT_TDA24, NOT_TDA23, NOT_TDA22, NOT_TDA21, NOT_TDA20;
  ...
  reg NOT_TAB1, NOT_TAB0, NOT_TDB25, NOT_TDB24, NOT_TDB23, NOT_TDB22, NOT_TDB21, NOT_TDB20;
 ...
 wire  CLKB_;
  ...
  assign `ARM_UD_DP DYA_ = RET1N_ ? (TENA_ ? DA_ : TDA_) : {26{1'bx}};
  ...
  assign `ARM_UD_DP DYB_ = RET1N_ ? (TENB_ ? DB_ : TDB_) : {26{1'bx}};
  assign `ARM_UD_SEQ QA_ = RET1N_ ? (BENA_ ? ((STOVA_ ? (QA_int_delayed) : (QA_int))) : TQA_) : {26{1'bx}};
  assign `ARM_UD_SEQ QB_ = RET1N_ ? (BENB_ ? ((STOVB_ ? (QB_int_delayed) : (QB_int))) : TQB_) : {26{1'bx}};
  ...
  begin
	$readmemb(filename, memld);
     if (CENA_ === 1'b1 && CENB_ === 1'b1) begin
      ...
      row = mem[row_address];
        writeEnable = {26{1'b1}};
        row_mask =  ( {15'b000000000000000, writeEnable[25], 15'b000000000000000, writeEnable[24],
        ...
        new_data =  ( {15'b000000000000000, wordtemp[25], 15'b000000000000000, wordtemp[24],
  ...
  begin
	dump_file_desc = $fopen(filename_dump);
     if (CENA_ === 1'b1 && CENB_ === 1'b1) begin
      ...
      QA_int = {shifted_readLatch0[100], shifted_readLatch0[96], shifted_readLatch0[92],
  ...
  begin
    if (RET1N_int === 1'bx || RET1N_int === 1'bz) begin
     ...
     && !CENA_int)} === 1'bx) begin
      ...
      QA_int = WENA_int !== 1'b1 ? QA_int : {26{1'bx}};
      QA_int_delayed = WENA_int !== 1'b1 ? QA_int_delayed : {26{1'bx}};
      ...
      if (WENA_int !== 1'b1) begin
        row_mask =  ( {15'b000000000000000, writeEnable[25], 15'b000000000000000, writeEnable[24],
          ...
          15'b000000000000000, writeEnable[3], 15'b000000000000000, writeEnable[2],
          15'b000000000000000, writeEnable[1], 15'b000000000000000, writeEnable[0]} << mux_address);
        new_data =  ( {15'b000000000000000, DA_int[25], 15'b000000000000000, DA_int[24],
  ...
  begin
    if (RET1N_int === 1'bx || RET1N_int === 1'bz) begin
     ...
     && !CENB_int)} === 1'bx) begin
      ...
      QB_int = WENB_int !== 1'b1 ? QB_int : {26{1'bx}};
      QB_int_delayed = WENB_int !== 1'b1 ? QB_int_delayed : {26{1'bx}};
      ...
      if (WENB_int !== 1'b1) begin
        row_mask =  ( {15'b000000000000000, writeEnable[25], 15'b000000000000000, writeEnable[24],
          ...
          15'b000000000000000, writeEnable[3], 15'b000000000000000, writeEnable[2],
          15'b000000000000000, writeEnable[1], 15'b000000000000000, writeEnable[0]} << mux_address);
        new_data =  ( {15'b000000000000000, DB_int[25], 15'b000000000000000, DB_int[24],
        ...
        data_out = (row >> (mux_address%4));
        readLatch1 = {data_out[412], data_out[408], data_out[404], data_out[400], data_out[396],
          data_out[392], data_out[388], data_out[384], data_out[380], data_out[376],
          data_out[372], data_out[368], data_out[364], data_out[360], data_out[356],
          data_out[352], data_out[348], data_out[344], data_out[340], data_out[336],
          data_out[332], data_out[328], data_out[324], data_out[320], data_out[316],
      ...
      QB_int = {shifted_readLatch1[100], shifted_readLatch1[96], shifted_readLatch1[92],
...
`endif
 ...
 wire  CLKB_;
  ...
  buf B32(DYA[20], DYA_[20]);
  buf B33(DYA[21], DYA_[21]);
  buf B34(DYA[22], DYA_[22]);
  buf B35(DYA[23], DYA_[23]);
  buf B36(DYA[24], DYA_[24]);
  buf B37(DYA[25], DYA_[25]);
  buf B38(CENYB, CENYB_);
  buf B39(WENYB, WENYB_);
  buf B40(AYB[0], AYB_[0]);
  buf B41(AYB[1], AYB_[1]);
  buf B42(AYB[2], AYB_[2]);
  buf B43(AYB[3], AYB_[3]);
  buf B44(AYB[4], AYB_[4]);
  buf B45(AYB[5], AYB_[5]);
  buf B46(AYB[6], AYB_[6]);
  buf B47(AYB[7], AYB_[7]);
  buf B48(AYB[8], AYB_[8]);
  buf B49(AYB[9], AYB_[9]);
  ...
  buf B84(QA[8], QA_[8]);
  buf B85(QA[9], QA_[9]);
  buf B86(QA[10], QA_[10]);
  buf B87(QA[11], QA_[11]);
  buf B88(QA[12], QA_[12]);
  buf B89(QA[13], QA_[13]);
  buf B90(QA[14], QA_[14]);
  buf B91(QA[15], QA_[15]);
  buf B92(QA[16], QA_[16]);
  buf B93(QA[17], QA_[17]);
  buf B94(QA[18], QA_[18]);
  buf B95(QA[19], QA_[19]);
  buf B96(QA[20], QA_[20]);
  buf B97(QA[21], QA_[21]);
  buf B98(QA[22], QA_[22]);
  buf B99(QA[23], QA_[23]);
  buf B100(QA[24], QA_[24]);
  buf B101(QA[25], QA_[25]);
  buf B102(QB[0], QB_[0]);
  buf B103(QB[1], QB_[1]);
  buf B104(QB[2], QB_[2]);
  buf B105(QB[3], QB_[3]);
  ...
  buf B110(QB[8], QB_[8]);
  buf B111(QB[9], QB_[9]);
  buf B112(QB[10], QB_[10]);
  buf B113(QB[11], QB_[11]);
  buf B114(QB[12], QB_[12]);
  buf B115(QB[13], QB_[13]);
  buf B116(QB[14], QB_[14]);
  buf B117(QB[15], QB_[15]);
  buf B118(QB[16], QB_[16]);
  buf B119(QB[17], QB_[17]);
  buf B120(QB[18], QB_[18]);
  buf B121(QB[19], QB_[19]);
  buf B122(QB[20], QB_[20]);
  buf B123(QB[21], QB_[21]);
  buf B124(QB[22], QB_[22]);
  buf B125(QB[23], QB_[23]);
  buf B126(QB[24], QB_[24]);
  buf B127(QB[25], QB_[25]);
  buf B128(CLKA_, CLKA);
  buf B129(CENA_, CENA);
  buf B130(WENA_, WENA);
  buf B131(AA_[0], AA[0]);
  ...
  buf B149(DA_[8], DA[8]);
  buf B150(DA_[9], DA[9]);
  buf B151(DA_[10], DA[10]);
  buf B152(DA_[11], DA[11]);
  buf B153(DA_[12], DA[12]);
  buf B154(DA_[13], DA[13]);
  buf B155(DA_[14], DA[14]);
  buf B156(DA_[15], DA[15]);
  buf B157(DA_[16], DA[16]);
  buf B158(DA_[17], DA[17]);
  buf B159(DA_[18], DA[18]);
  buf B160(DA_[19], DA[19]);
  buf B161(DA_[20], DA[20]);
  buf B162(DA_[21], DA[21]);
  buf B163(DA_[22], DA[22]);
  buf B164(DA_[23], DA[23]);
  buf B165(DA_[24], DA[24]);
  buf B166(DA_[25], DA[25]);
  buf B167(CLKB_, CLKB);
  buf B168(CENB_, CENB);
  buf B169(WENB_, WENB);
  buf B170(AB_[0], AB[0]);
  ...
  buf B188(DB_[8], DB[8]);
  buf B189(DB_[9], DB[9]);
  buf B190(DB_[10], DB[10]);
  buf B191(DB_[11], DB[11]);
  buf B192(DB_[12], DB[12]);
  buf B193(DB_[13], DB[13]);
  buf B194(DB_[14], DB[14]);
  buf B195(DB_[15], DB[15]);
  buf B196(DB_[16], DB[16]);
  buf B197(DB_[17], DB[17]);
  buf B198(DB_[18], DB[18]);
  buf B199(DB_[19], DB[19]);
  buf B200(DB_[20], DB[20]);
  buf B201(DB_[21], DB[21]);
  buf B202(DB_[22], DB[22]);
  buf B203(DB_[23], DB[23]);
  buf B204(DB_[24], DB[24]);
  buf B205(DB_[25], DB[25]);
  buf B206(EMAA_[0], EMAA[0]);
  buf B207(EMAA_[1], EMAA[1]);
  buf B208(EMAA_[2], EMAA[2]);
  buf B209(EMAWA_[0], EMAWA[0]);
  buf B210(EMAWA_[1], EMAWA[1]);
  buf B211(EMASA_, EMASA);
  buf B212(EMAB_[0], EMAB[0]);
  buf B213(EMAB_[1], EMAB[1]);
  buf B214(EMAB_[2], EMAB[2]);
  buf B215(EMAWB_[0], EMAWB[0]);
  buf B216(EMAWB_[1], EMAWB[1]);
  buf B217(EMASB_, EMASB);
  buf B218(TENA_, TENA);
  buf B219(BENA_, BENA);
  buf B220(TCENA_, TCENA);
  buf B221(TWENA_, TWENA);
  buf B222(TAA_[0], TAA[0]);
  buf B223(TAA_[1], TAA[1]);
  buf B224(TAA_[2], TAA[2]);
  buf B225(TAA_[3], TAA[3]);
  buf B226(TAA_[4], TAA[4]);
  buf B227(TAA_[5], TAA[5]);
  buf B228(TAA_[6], TAA[6]);
  buf B229(TAA_[7], TAA[7]);
  buf B230(TAA_[8], TAA[8]);
  buf B231(TAA_[9], TAA[9]);
  buf B232(TDA_[0], TDA[0]);
  buf B233(TDA_[1], TDA[1]);
  buf B234(TDA_[2], TDA[2]);
  buf B235(TDA_[3], TDA[3]);
  buf B236(TDA_[4], TDA[4]);
  buf B237(TDA_[5], TDA[5]);
  buf B238(TDA_[6], TDA[6]);
  buf B239(TDA_[7], TDA[7]);
  buf B240(TDA_[8], TDA[8]);
  buf B241(TDA_[9], TDA[9]);
  buf B242(TDA_[10], TDA[10]);
  buf B243(TDA_[11], TDA[11]);
  buf B244(TDA_[12], TDA[12]);
  buf B245(TDA_[13], TDA[13]);
  buf B246(TDA_[14], TDA[14]);
  buf B247(TDA_[15], TDA[15]);
  buf B248(TDA_[16], TDA[16]);
  buf B249(TDA_[17], TDA[17]);
  buf B250(TDA_[18], TDA[18]);
  buf B251(TDA_[19], TDA[19]);
  buf B252(TDA_[20], TDA[20]);
  buf B253(TDA_[21], TDA[21]);
  buf B254(TDA_[22], TDA[22]);
  buf B255(TDA_[23], TDA[23]);
  buf B256(TDA_[24], TDA[24]);
  buf B257(TDA_[25], TDA[25]);
  buf B258(TQA_[0], TQA[0]);
  buf B259(TQA_[1], TQA[1]);
  buf B260(TQA_[2], TQA[2]);
  buf B261(TQA_[3], TQA[3]);
  buf B262(TQA_[4], TQA[4]);
  buf B263(TQA_[5], TQA[5]);
  buf B264(TQA_[6], TQA[6]);
  buf B265(TQA_[7], TQA[7]);
  buf B266(TQA_[8], TQA[8]);
  buf B267(TQA_[9], TQA[9]);
  buf B268(TQA_[10], TQA[10]);
  buf B269(TQA_[11], TQA[11]);
  buf B270(TQA_[12], TQA[12]);
  buf B271(TQA_[13], TQA[13]);
  buf B272(TQA_[14], TQA[14]);
  buf B273(TQA_[15], TQA[15]);
  buf B274(TQA_[16], TQA[16]);
  buf B275(TQA_[17], TQA[17]);
  buf B276(TQA_[18], TQA[18]);
  buf B277(TQA_[19], TQA[19]);
  buf B278(TQA_[20], TQA[20]);
  buf B279(TQA_[21], TQA[21]);
  buf B280(TQA_[22], TQA[22]);
  buf B281(TQA_[23], TQA[23]);
  buf B282(TQA_[24], TQA[24]);
  buf B283(TQA_[25], TQA[25]);
  buf B284(TENB_, TENB);
  buf B285(BENB_, BENB);
  buf B286(TCENB_, TCENB);
  buf B287(TWENB_, TWENB);
  buf B288(TAB_[0], TAB[0]);
  buf B289(TAB_[1], TAB[1]);
  buf B290(TAB_[2], TAB[2]);
  buf B291(TAB_[3], TAB[3]);
  buf B292(TAB_[4], TAB[4]);
  buf B293(TAB_[5], TAB[5]);
  buf B294(TAB_[6], TAB[6]);
  buf B295(TAB_[7], TAB[7]);
  buf B296(TAB_[8], TAB[8]);
  buf B297(TAB_[9], TAB[9]);
  buf B298(TDB_[0], TDB[0]);
  buf B299(TDB_[1], TDB[1]);
  buf B300(TDB_[2], TDB[2]);
  buf B301(TDB_[3], TDB[3]);
  buf B302(TDB_[4], TDB[4]);
  buf B303(TDB_[5], TDB[5]);
  buf B304(TDB_[6], TDB[6]);
  buf B305(TDB_[7], TDB[7]);
  buf B306(TDB_[8], TDB[8]);
  buf B307(TDB_[9], TDB[9]);
  buf B308(TDB_[10], TDB[10]);
  buf B309(TDB_[11], TDB[11]);
  buf B310(TDB_[12], TDB[12]);
  buf B311(TDB_[13], TDB[13]);
  buf B312(TDB_[14], TDB[14]);
  buf B313(TDB_[15], TDB[15]);
  buf B314(TDB_[16], TDB[16]);
  buf B315(TDB_[17], TDB[17]);
  buf B316(TDB_[18], TDB[18]);
  buf B317(TDB_[19], TDB[19]);
  buf B318(TDB_[20], TDB[20]);
  buf B319(TDB_[21], TDB[21]);
  buf B320(TDB_[22], TDB[22]);
  buf B321(TDB_[23], TDB[23]);
  buf B322(TDB_[24], TDB[24]);
  buf B323(TDB_[25], TDB[25]);
  buf B324(TQB_[0], TQB[0]);
  buf B325(TQB_[1], TQB[1]);
  buf B326(TQB_[2], TQB[2]);
  buf B327(TQB_[3], TQB[3]);
  buf B328(TQB_[4], TQB[4]);
  buf B329(TQB_[5], TQB[5]);
  buf B330(TQB_[6], TQB[6]);
  buf B331(TQB_[7], TQB[7]);
  buf B332(TQB_[8], TQB[8]);
  buf B333(TQB_[9], TQB[9]);
  buf B334(TQB_[10], TQB[10]);
  buf B335(TQB_[11], TQB[11]);
  buf B336(TQB_[12], TQB[12]);
  buf B337(TQB_[13], TQB[13]);
  buf B338(TQB_[14], TQB[14]);
  buf B339(TQB_[15], TQB[15]);
  buf B340(TQB_[16], TQB[16]);
  buf B341(TQB_[17], TQB[17]);
  buf B342(TQB_[18], TQB[18]);
  buf B343(TQB_[19], TQB[19]);
  buf B344(TQB_[20], TQB[20]);
  buf B345(TQB_[21], TQB[21]);
  buf B346(TQB_[22], TQB[22]);
  buf B347(TQB_[23], TQB[23]);
  buf B348(TQB_[24], TQB[24]);
  buf B349(TQB_[25], TQB[25]);
  buf B350(RET1N_, RET1N);
  buf B351(STOVA_, STOVA);
  buf B352(STOVB_, STOVB);
  buf B353(COLLDISN_, COLLDISN);
  ...
  assign DYA_ = RET1N_ ? (TENA_ ? DA_ : TDA_) : {26{1'bx}};
  ...
  assign DYB_ = RET1N_ ? (TENB_ ? DB_ : TDB_) : {26{1'bx}};
   `ifdef ARM_FAULT_MODELING
     sram_FIFO_NMS_error_injection u1(.CLK(CLKA_), .Q_out(QA_), .A(AA_int), .CEN(CENA_int), .TQ(TQA_), .BEN(BENA_), .WEN(WENA_int), .Q_in(QA_int));
  `else
  assign QA_ = RET1N_ ? (BENA_ ? ((STOVA_ ? (QA_int_delayed) : (QA_int))) : TQA_) : {26{1'bx}};
  `endif
  assign QB_ = RET1N_ ? (BENB_ ? ((STOVB_ ? (QB_int_delayed) : (QB_int))) : TQB_) : {26{1'bx}};
...
`endif
`timescale 1ns/1ps
module sram_FIFO_NMS_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, BEN, TQ);
   output [25:0] Q_out;
   input [25:0] Q_in;
   input CLK;
   input [9:0] A;
...
begin
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.SMARCHCHKBVCD_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(10'd37,5'd22,2'd1,2'd0);
   `endif
end
   task add_fault;
   //This task injects fault in memory
   //In order to inject fault in redundant column for Bit 0 to 12, column address
   //should have value in range of 12 to 15
   //In order to inject fault in redundant column for Bit 13 to 25, column address
